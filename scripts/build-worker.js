...
const sharingPath = path.join(root, 'sharing.md');
const sharingMd = fs.existsSync(sharingPath)
  ? fs.readFileSync(sharingPath, 'utf8')
  : '# How I talk to Cursor\n\n(No entries yet.)\n';
// Also publish into the static public folder for local/preview use
fs.mkdirSync(path.join(root, 'cloudflare', 'public'), { recursive: true });
fs.writeFileSync(path.join(root, 'cloudflare', 'public', 'sharing.md'), sharingMd);
...
const SHARING_MD = ${JSON.stringify(sharingMd)};
  ...
  async fetch(request, env) {
    ...
    if (request.method === 'GET' && pathName === '/health') {
      return json(200, {
        ...
        routes: ['/', '/health', '/chat', '/tts', '/stt', '/stt-token', '/sharing.md', 'cursor proxy via x-cursor-path']
      });
    }
    ...
    if (
      ...
      (pathName === '/sharing.md' || pathName === '/sharing')
    ) {
      const body = request.method === 'HEAD' ? null : SHARING_MD;
